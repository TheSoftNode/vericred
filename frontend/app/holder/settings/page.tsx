...
import { User, Bell, Key, Save, CheckCircle2, Shield } from "lucide-react";
...
export default function HolderSettingsPage() {
  ...
  const [credentialReceived, setCredentialReceived] = useState(true);
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-950/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-white">Credential Received</p>
                    <p className="text-xs text-slate-400">New credentials notifications</p>
                  </div>
                  <button
                    onClick={() => setCredentialReceived(!credentialReceived)}
                    className={`relative w-12 h-6 rounded-full transition-colors ${
                      credentialReceived ? "bg-emerald-500" : "bg-white/10"
                    }`}
                  >
                    <motion.div
                      animate={{ x: credentialReceived ? 24 : 2 }}
                      className="absolute top-1 w-4 h-4 bg-white rounded-full"
                    />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Verifier access requests</p>
                  </div>
                  <button
                    onClick={() => setAccessRequests(!accessRequests)}
                    className={`relative w-12 h-6 rounded-full transition-colors ${
                      accessRequests ? "bg-emerald-500" : "bg-white/10"
                    }`}
                  >
                    <motion.div
                      animate={{ x: accessRequests ? 24 : 2 }}
                      className="absolute top-1 w-4 h-4 bg-white rounded-full"
                    />
                  </button>
                </div>
